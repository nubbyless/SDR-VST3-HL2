...
static void runtime_log(const wchar_t* msg)
{
...
namespace
{
	...
	const DWORD kRuntimeHostCommandTimeoutMs = 30000;
	const DWORD kRuntimeHostShutdownTimeoutMs = 10000;
	...
	void release_runtime(VstPluginRuntime& runtime)
	{
		...
		/*
		Skip module.reset() (FreeLibrary): some VST3 plugins (notably WaveShell
		hosts) deadlock inside DllMain(DLL_PROCESS_DETACH) when their loaded
		sub-plugins are still referenced.  This causes the host thread to hang
		indefinitely, preventing command_complete_event from being signalled.
		The bridge-side VstRuntime_Destroy applies a no-op deleter as a safety
		net to prevent the shared_ptr destructor from also calling FreeLibrary.
		Leaked DLL handles are reclaimed by the OS on process exit.
		*/
	...
	int process_host_command(VstPluginRuntime& runtime)
	{
		...
		*/
		...
		case VstPluginRuntime::HostCommandShutdown:
			/*
			Exit the host thread immediately. Do NOT call release_runtime(),
			base->terminate(), or any plugin method — WaveShell and other
			plugins deadlock in their DllMain / terminate when the loader
			lock is corrupted or when sub-plugins are still referenced.
			All cleanup is handled by VstRuntime_Destroy on the caller
			side (no-op FreeLibrary deleter, handle closure, struct delete).
			*/
			runtime.editor_session = 0;
			return 0;
		}
	...
	const wchar_t* plugin_cid)
{
	...
	{
		wchar_t dbg[512];
		swprintf_s(dbg, L"[runtime] VstRuntime_Create _beginthreadex handle=%p thread_id=%lu path=\"%s\"\r\n",
			reinterpret_cast<void*>(thread_handle), static_cast<unsigned long>(thread_id), plugin_path);
		runtime_log(dbg);
	}
	if (!thread_handle)
	{
		runtime_log(L"[runtime] VstRuntime_Create _beginthreadex FAILED\r\n");
		CloseHandle(new_runtime->host_ready_event);
		CloseHandle(new_runtime->command_event);
		CloseHandle(new_runtime->command_complete_event);
		DeleteCriticalSection(&new_runtime->api_lock);
	...
	new_runtime->host_thread_id = thread_id;
	{
		...
		{
			wchar_t dbg[512];
			swprintf_s(dbg, L"[runtime] VstRuntime_Create startup_wait=%lu timeout=%lu path=\"%s\"\r\n",
				static_cast<unsigned long>(startup_wait), static_cast<unsigned long>(kRuntimeHostCommandTimeoutMs), plugin_path);
			runtime_log(dbg);
		}
		if (startup_wait != WAIT_OBJECT_0)
		{
			...
			{
				runtime_log(L"[runtime] VstRuntime_Create TIMEOUT — terminating thread\r\n");
				TerminateThread(new_runtime->host_thread, 1);
			}
			else
			{
				runtime_log(L"[runtime] VstRuntime_Create thread exited during startup\r\n");
			}
	...
	{
		wchar_t dbg[512];
		swprintf_s(dbg, L"[runtime] VstRuntime_Create INITIALIZE result=%d path=\"%s\"\r\n",
			result, plugin_path);
		runtime_log(dbg);
	}
	if (result != 0)
	{
		runtime_log(L"[runtime] VstRuntime_Create INITIALIZE FAILED — shutting down thread\r\n");
		EnterCriticalSection(&new_runtime->command_lock);
		...
		{
			runtime_log(L"[runtime] VstRuntime_Create INITIALIZE cleanup TIMEOUT — TerminateThread\r\n");
			TerminateThread(new_runtime->host_thread, 1);
		}
		WaitForSingleObject(new_runtime->host_thread, 500);
		CloseHandle(new_runtime->host_thread);
...
void VstProcessingState_Release(VstProcessingState*& state)
{
	...
	void safe_destroy_runtime(VstPluginRuntime* runtime)
	{
		__try
		{
			delete runtime;
		...
		{
			runtime_log(L"[runtime] VstRuntime_Destroy delete crashed, leaking runtime struct\r\n");
		}
	}
	...
	void VstRuntime_Destroy(VstPluginRuntime*& runtime)
	{
		...
		{
			runtime_log(L"[runtime] VstRuntime_Destroy SHUTDOWN TIMEOUT — calling TerminateThread (loader lock may be corrupted!)\r\n");
		...
		/*
		Prevent FreeLibrary: the module shared_ptr destructor would call
		FreeLibrary, which can deadlock (loader lock corrupted after
		TerminateThread, or plugin DllMain(DLL_PROCESS_DETACH) hangs).
		Replace the owning shared_ptr with a non-owning alias so the
		destructor is a no-op.  Leaked DLL handles are reclaimed by the
		OS on process exit.
		*/
		if (local_runtime->module) {
			VST3::Hosting::Module* raw_module = local_runtime->module.get();
			local_runtime->module = std::shared_ptr<VST3::Hosting::Module>(
				raw_module, [](VST3::Hosting::Module*) {});
		}

		/*
		Release COM smart pointers before delete: IPtr destructors call
		release() on the VST3 COM interfaces.  If the plugin DLL is in a
		corrupted state (e.g. loader lock broken by TerminateThread), this
		can ACCESS_VIOLATE.  Use memset to zero the internal pointers so
		the IPtr destructors see nullptr and skip the release() call.
		Leaking COM references here is harmless — the DLL module is
		already leaked and the OS reclaims everything on process exit.
		*/
		local_runtime->host_application.reset();
		memset(&local_runtime->processor, 0, sizeof(local_runtime->processor));
		memset(&local_runtime->controller, 0, sizeof(local_runtime->controller));
		memset(&local_runtime->component, 0, sizeof(local_runtime->component));

		safe_destroy_runtime(local_runtime);
	}
