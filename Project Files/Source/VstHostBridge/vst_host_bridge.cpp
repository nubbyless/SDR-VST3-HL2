...
namespace
{
	...
	static void log_to_file(const wchar_t* msg)
	{
		...
		{
			wcscat_s(path, L"ThetisVstBridge.log");
			FILE* f = 0;
	...
	LONG query_host_status(VstChainKind kind)
	{
		...
		{
			DWORD exit_code = 0;
			GetExitCodeProcess(local_process_handle, &exit_code);
			{
				wchar_t dbg[256];
				swprintf_s(dbg, L"[bridge] HOST CRASHED exit_code=%lu\r\n", exit_code);
				log_to_file(dbg);
			}
	...
	bool send_host_request(VstChainKind kind, VstHostIpcPacket& request, VstHostIpcPacket& response, DWORD timeout_ms = kControlPipeIoTimeoutMs)
	{
		...
		if (request_failed && state.control_pipe_handle != INVALID_HANDLE_VALUE)
		{
			{
				wchar_t dbg2[256];
				swprintf_s(dbg2, L"[bridge] send_host_request FAILED kind=%s type=%d closing pipe\r\n",
					host_kind_name(kind), request.header.type);
				log_to_file(dbg2);
			}
			CloseHandle(state.control_pipe_handle);
			state.control_pipe_handle = INVALID_HANDLE_VALUE;
	...
	bool apply_cached_chain_snapshot(VstChainKind kind)
	{
		...
		*/
		...
		for (int i = 0; i < static_cast<int>(cached_plugins.size()); ++i)
		{
			...
			if (!send_host_request(kind, request, response, 120000) || response.result < 0)
				return false;
			...
			{
				host_set_plugin_state(kind, host_index, cached_plugin.state_data.data(), static_cast<int>(cached_plugin.state_data.size()));
			}
...
unsigned __stdcall restart_host_thread_proc(void* context)
{
	...
	void shutdown_host_process(VstChainKind kind)
	{
		...
		{
			wchar_t dbg[256];
			swprintf_s(dbg, L"[bridge] shutdown_host_process kind=%s sending SHUTDOWN\r\n", host_kind_name(kind));
			log_to_file(dbg);
		}
		...
		{
			DWORD wait_result = WaitForSingleObject(state.process_handle, 2000);
			if (wait_result == WAIT_TIMEOUT)
			{
				wchar_t dbg[256];
				swprintf_s(dbg, L"[bridge] shutdown_host_process kind=%s TIMEOUT, TerminateProcess\r\n", host_kind_name(kind));
				log_to_file(dbg);
			...
			{
				wchar_t dbg[256];
				swprintf_s(dbg, L"[bridge] shutdown_host_process kind=%s exited cleanly wait=%lu\r\n",
					host_kind_name(kind), wait_result);
				log_to_file(dbg);
			}
		}
...
int VST_AddPlugin(VstChainKind kind, const wchar_t* plugin_path, const wchar_t* plugin_cid)
{
	{
		wchar_t buf[512];
		swprintf_s(buf, L"[bridge] VST_AddPlugin ENTER kind=%d path=\"%s\"\r\n",
			(int)kind, plugin_path ? plugin_path : L"(null)");
		log_to_file(buf);
	...
	trace_oop(
		L"[VST bridge %s] VST_AddPlugin ENTER path=\"%s\" cid=\"%s\" cmd_avail=%d cached_created=%d\r\n",
		host_kind_name(kind),
		plugin_path ? plugin_path : L"",
		plugin_cid ? plugin_cid : L"",
		host_command_available(kind) ? 1 : 0,
		host_state.cached_created);

	{
		wchar_t buf[256];
		swprintf_s(buf, L"[bridge] cmd_avail=%d cached_created=%d\r\n",
			host_command_available(kind) ? 1 : 0, host_state.cached_created);
		log_to_file(buf);
	}
	...
	if (host_command_available(kind))
	{
		...
		if (!host_state.cached_created)
		{
			int create_result = host_command_simple(kind, VST_HOST_IPC_CREATE_CHAIN, 48000, 1024, 2, 0.0);
			{
				wchar_t buf[256];
				swprintf_s(buf, L"[bridge] auto-create chain result=%d\r\n", create_result);
				log_to_file(buf);
			}
			if (create_result == 0)
			{
				AcquireSRWLockExclusive(&host_state.cache_lock);
				host_state.cached_created = 1;
				host_state.cached_sample_rate = 48000;
				host_state.cached_max_block_size = 1024;
				host_state.cached_num_channels = 2;
				advance_cache_generation(host_state);
				ReleaseSRWLockExclusive(&host_state.cache_lock);
			}
		}
		...
		if (!send_host_request(kind, request, response, 120000) || response.result < 0)
		{
			{
				wchar_t buf[256];
				swprintf_s(buf, L"[bridge] ADD_PLUGIN FAILED result=%d\r\n", response.result);
				log_to_file(buf);
			}
		...
		{
			wchar_t buf[256];
			swprintf_s(buf, L"[bridge] ADD_PLUGIN OK index=%d\r\n", plugin_index);
			log_to_file(buf);
		}

		initialize_cached_plugin_state(cached_plugin, plugin_path, plugin_cid);
		{
			int info_result = host_get_plugin_info(kind, plugin_index, host_info);
			cached_plugin.load_state = info_result == 0 ? host_info.load_state : VST_PLUGIN_LOAD_DESCRIPTOR_ONLY;
			{
				wchar_t buf[256];
				swprintf_s(buf, L"[bridge] after-add info_result=%d host_load=%d cached_load=%d\r\n",
					info_result, host_info.load_state, cached_plugin.load_state);
				log_to_file(buf);
			}
			trace_oop(
				L"[VST bridge %s] after add plugin: info_result=%d host_load_state=%d cached_load=%d\r\n",
				host_kind_name(kind),
				info_result,
				host_info.load_state,
	...
	else
	{
		...
		{
			wchar_t buf[256];
			swprintf_s(buf, L"[bridge] OFFLINE host not available, cache-only\r\n");
			log_to_file(buf);
		}
		trace_oop(
			L"[VST bridge %s] VST_AddPlugin OFFLINE path=\"%s\" (host not available)\r\n",
			host_kind_name(kind),
			plugin_path ? plugin_path : L"");
		initialize_cached_plugin_state(cached_plugin, plugin_path, plugin_cid);
		ensure_host_recovery(kind);
...
int VST_RemovePlugin(VstChainKind kind, int index)
{
	...
	int cached_count = static_cast<int>(host_state.cached_plugins.size());
	ReleaseSRWLockShared(&host_state.cache_lock);
	{
		wchar_t buf[256];
		swprintf_s(buf, L"[bridge] RemovePlugin kind=%s index=%d cached_count=%d cmd_avail=%d\r\n",
			host_kind_name(kind), index, cached_count, host_command_available(kind) ? 1 : 0);
		log_to_file(buf);
	}
	if (!index_valid)
		return -1;
	if (host_command_available(kind))
	{
		...
		{
			wchar_t buf[256];
			swprintf_s(buf, L"[bridge] RemovePlugin IPC result=%d\r\n", host_result);
			log_to_file(buf);
		}
		if (host_result != 0)
			return host_result;
	...
	{
		wchar_t buf[256];
		swprintf_s(buf, L"[bridge] RemovePlugin host NOT available, calling ensure_host_recovery\r\n");
		log_to_file(buf);
		ensure_host_recovery(kind);
	}
	...
	{
		wchar_t buf[256];
		swprintf_s(buf, L"[bridge] RemovePlugin cache updated, new count=%d\r\n", VST_GetPluginCount(kind));
		log_to_file(buf);
	}
	trace_oop(
...
int VST_GetPluginInfo(VstChainKind kind, int index, VstPluginInfo* info)
{
	...
	if (result == 0)
	{
		{
			wchar_t buf[300];
			swprintf_s(buf, L"[bridge] GetPluginInfo idx=%d load=%d name=\"%s\"\r\n",
				index, info->load_state, info->name);
			log_to_file(buf);
		}
		trace_oop(
