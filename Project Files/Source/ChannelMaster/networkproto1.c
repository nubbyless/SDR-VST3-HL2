...

				{
					...
					for (frame = 0; frame < 2; frame++)
					{
						...
						if ((bptr[0] == 0x7f) && (bptr[1] == 0x7f) && (bptr[2] == 0x7f))
						{
							...
							for (isamp = 0; isamp < spr; isamp++)							// for each sample set
							{
								int k = 8 + nddc * 6 + isamp * (2 + nddc * 6);
...
DWORD WINAPI sendProtocol1Samples(LPVOID n)
{
	...
	while (io_keep_running != 0)
	{
		...
		if (prn->swap_audio_channels)				// MI0BOT: To cater for different firmware at the hardware, allow control of audio channels swapping
		{
