...
static void host_log(const wchar_t* msg)
{
...
static LONG WINAPI host_crash_filter(EXCEPTION_POINTERS* ep)
{
	wchar_t dbg[512];
	const wchar_t* code_name = L"UNKNOWN";
	if (ep && ep->ExceptionRecord)
	{
		switch (ep->ExceptionRecord->ExceptionCode)
		{
		case EXCEPTION_ACCESS_VIOLATION: code_name = L"ACCESS_VIOLATION"; break;
		case EXCEPTION_STACK_OVERFLOW: code_name = L"STACK_OVERFLOW"; break;
		case EXCEPTION_INT_DIVIDE_BY_ZERO: code_name = L"INT_DIVIDE_BY_ZERO"; break;
		case EXCEPTION_ILLEGAL_INSTRUCTION: code_name = L"ILLEGAL_INSTRUCTION"; break;
		case EXCEPTION_INVALID_DISPOSITION: code_name = L"INVALID_DISPOSITION"; break;
		case 0xE06D7363: code_name = L"C++_EXCEPTION"; break;
		}
		swprintf_s(dbg, L"[host] *** CRASH *** code=0x%08X (%s) addr=%p\r\n",
			ep->ExceptionRecord->ExceptionCode, code_name,
			ep->ExceptionRecord->ExceptionAddress);
	}
	else
	{
		swprintf_s(dbg, L"[host] *** CRASH *** null exception pointers\r\n");
	}
	host_log(dbg);
...
namespace
{
	...
	void handle_chain_command(const VstHostIpcPacket& request, VstHostIpcPacket& response)
	{
		...
		case VST_HOST_IPC_ADD_PLUGIN:
			response.result = VstChain_AddPlugin(g_chains[kind], request.plugin_path, request.plugin_cid);
			bump_snapshot_generation_now = response.result >= 0;
			break;
...
int RunVstAudioHost(HINSTANCE instance, PWSTR command_line)
{
	...
	SetUnhandledExceptionFilter(host_crash_filter);
	...
	for (;;)
	{
		...
		{
			wchar_t dbg[256];
			swprintf_s(dbg, L"[host] FATAL_RUNTIME_FAILURE result=%d type=%d\r\n",
				response.result, request.header.type);
			host_log(dbg);
			exit_code = 20;
			break;
		}
...
cleanup:
	...
	{
		wchar_t dbg[128];
		swprintf_s(dbg, L"[host] RunVstAudioHost EXIT exit_code=%d chain=%s\r\n", exit_code,
			chain_kind < VST_CHAIN_COUNT ? (chain_kind == 0 ? L"rx" : L"tx") : L"??");
		host_log(dbg);
	}
